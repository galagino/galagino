...
void mooncresta::prepare_frame(void) {
  ...
  */
  ...
  for(int idx = 0; idx < 8; idx++) {
    unsigned char *bbase = memory + MC_OFF_SPRITERAM + 0x0060 + idx * 4;
...
void mooncresta::blit_tile_scroll(short row, signed char col, unsigned char scroll) {
  if((row < 2) || (row >= 34))
    return;
...
void mooncresta::render_row(short row) {
  ...
  // ObjRAM even bytes at 0x9800+2*col → memory[0x0C00+2*(row-2)]
  unsigned char scroll = memory[MC_OFF_SPRITERAM + 2 * (row - 2)];
