...
void todruaga::reset() {
  ...
  // Lazy alloc: ~47KB DRAM interna al primo avvio (vedi mappy.cpp per la
  // storia: le due 6809 + il render sfrattano la cache flash su entrambi i
  // core). Mai liberata, come Phoenix/mappy.
  if (!rom_cached && false) {
    const uint32_t CAPS = MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT;
    unsigned char *rm = (unsigned char *)heap_caps_malloc(0x8000, CAPS);
    unsigned char *rs = (unsigned char *)heap_caps_malloc(0x2000, CAPS);
    // tilemap 4KB + cmap tiles/prio 2x512B + cmap sprite 2KB in un blocco
    unsigned char *gx = (unsigned char *)heap_caps_malloc(4096 + 2 * 512 + 2048, CAPS);
    if (rm && rs && gx) {
      memcpy(rm, todruaga_rom_main, 0x8000);
      memcpy(rs, todruaga_rom_sub, 0x2000);
      memcpy(gx,        todruaga_tilemap,             4096);
      memcpy(gx + 4096, todruaga_colormap_tiles,       512);
      memcpy(gx + 4608, todruaga_colormap_tiles_prio,  512);
      memcpy(gx + 5120, todruaga_colormap_sprites,    2048);
      rom_main = rm;
      rom_sub = rs;
